...
- ~~**`pod install` de la app de ejemplo de la librería** en este entorno: la ruta del proyecto contiene un espacio (`.../react native/examen-rn/...`)~~ — resuelto: no era el espacio en la ruta, era que faltaba `bundle install` (gems de CocoaPods locales del `Gemfile` del ejemplo). Con `bundle install && bundle exec pod install` en `libreria/rn-savings-notifier/example/ios` instala los 74 pods sin error, prebuilt binaries de RN 0.85 incluidos. Detalle en el [README de la librería](libreria/rn-savings-notifier/README.md#pendientes).
...
### Fricciones de entorno encontradas grabando el demo (ambas resueltas, `mobile/` sí corre)

- **Android — build fallaba con `Configuring project ':react-native-safe-area-context' without an existing directory`**: no es un problema del proyecto ni del espacio en la ruta. El directorio del repo se renombró en algún momento (de una ruta con espacio a la actual con guion) mientras un Gradle Daemon y unos Kotlin compile daemons de una sesión anterior seguían vivos; esos daemons cachean `user.dir` al arrancar y no lo refrescan, así que seguían resolviendo rutas con el nombre viejo. `android/build/generated/autolinking/autolinking.json` (cacheado por SHA de lockfiles, ver `ReactSettingsExtension.kt` del gradle-plugin de RN) quedó con esa ruta vieja y no se regeneraba porque los lockfiles no habían cambiado. Fix: `./gradlew --stop` + matar los Kotlin daemons (`pkill -f KotlinCompileDaemon`) + borrar `android/build`, `android/app/build` y `android/app/.cxx` para forzar regeneración. Un `npm run android` limpio en una máquina nueva no lo sufre — es puramente una cicatriz de haber movido el repo con daemons corriendo.
- **iOS — RedBox fatal `Unable to find module for SampleTurboModule` al arrancar**: bug de empaquetado de React Native 0.87.0, no del proyecto. `node_modules/react-native/package.json` declara `SampleTurboModule` (un módulo de ejemplo interno de RN, pensado solo para RNTester) en `codegenConfig.libraries[0].ios.modules` con `unstableRequiresMainQueueSetup: true`; eso hace que **todo** proyecto RN 0.87.0 genere `RCTUnstableModulesRequiringMainQueueSetupProvider.mm` con ese módulo listado para inicialización eager, pero su implementación nativa (`ReactCommon/react/nativemodule/samples/`) no forma parte del `React-Core.podspec` público — el lookup falla y `RCTTurboModuleManager` lo escala a `RCTLogError`, que en dev build es RedBox de pantalla completa (no un warning descartable). Workaround aplicado en este entorno: quitar la entrada `"SampleTurboModule"` de `codegenConfig.libraries[0].ios.modules` en `node_modules/react-native/package.json` y correr `pod install` de nuevo para regenerar el provider sin ese módulo. Es un cambio en `node_modules` (no versionado, se pierde con cada `npm install`); quien reproduzca el demo en limpio necesita repetirlo, o simplemente tocar "Dismiss" en el RedBox — la app funciona normal por debajo, el error es solo de un módulo de ejemplo que nadie usa.
